import { effect, inject, linkedSignal, Signal, WritableSignal } from '@angular/core';
...
export function equal<T extends Record<string, any>>(a: T, b: T): boolean {
...
export abstract class GenricBaseFormState<T extends Record<string, any>> {
  protected abstract readonly state: Signal<T>;

  protected abstract readonly schemaOrOptions: SchemaOrSchemaFn<T>;

  abstract set(_patch: DeepPartial<T>): void;
  ...
  form(): FieldTree<T> {
    const _formModel = linkedSignal(() => this.state(), {
      equal: this.equal,
    });

    effect(() => {
      const state = _formModel();
      if (!this.equal(this.state(), state)) this.set(state);
    });

    return form(_formModel, this.schemaOrOptions);
  }
}

export abstract class GenricFormState<
